...
def main() -> None:
    ...
    command = [
        ...
        "--fps", str(config["physics"]["fps"]),
        "--force_magnitude", str(config["physics"]["force_magnitude_g"]),
        "--initial_grasp_duration",
        str(config["physics"]["initial_grasp_duration_s"]),
        "--tug_sequences", json.dumps(config["physics"]["tug_sequences"]),
