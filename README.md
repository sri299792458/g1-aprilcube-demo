...
| Task | Entry point |
| --- | --- |
| Understand frames and descriptors | [GraspGenX contract](docs/graspgenx_contract.md), [descriptor evidence](docs/dex3_rev1_descriptor.md), `tools/build_dex3_rev1_descriptors.py` |
| Build or inspect qualified candidates | [Grasp pipeline](docs/grasp-pipeline.md), `g1_aprilcube_demo/grasping/` |
| Follow assembly planning | [Runtime assembly](docs/runtime-assembly.md), `g1_aprilcube_demo/planning/runtime_assembly.py` |
| Inspect the later 40 mm cube assets | [Cube pool](artifacts/grasp_atlas/cube40_viral_v1/right/arm_grasp_pool.yaml), [tripod fixture study](hardware/fixtures/cube_tripod_presenter/README.md) |
| Understand the physical research workflow | [G1 research guide](https://sri299792458.github.io/g1-research-docs/) |

The 45 mm T/U/cube assembly study and the later **40 mm printed cube** use
different geometry and independently generated grasps. The 40 mm pool contains
3,178 retained candidates. Its `isaac_closed_q` values record achieved simulated
joint states; they are not hardware closing targets. The later tabletop work
superseded that interpretation and removed the tripod from its stacking setup.
...
The implemented interfaces and limitations are described in
[runtime assembly](docs/runtime-assembly.md). Attachment transitions are
symbolic; the recorded plan does not validate magnetic hardware or physical
G1 assembly.
...
These commands require generated robot/part assets and the referenced grasp
pools. See [input availability](docs/runtime-assembly.md#input-availability)
before running them on a fresh checkout.
...
`third_party/GraspGenX/assets/x_grippers/`; the builder writes the numerical
audit to `artifacts/dex3_rev1_descriptor/audit.json` (generated locally).
...
[the grasp-pipeline guide](docs/grasp-pipeline.md)
...
Keep the candidate geometry, original `object_T_G`, source identifiers,
qualification settings and test outcome together. A changed physics profile
cannot inherit another profile's pass count. Preserve failures in technical
reports and distinguish simulated retention from kinematic replay and robot
execution.

The public tree retains the code, configuration, seven selected grasp/provenance
artifacts, fixture assets and technical result reports. Generated outputs and
model downloads remain outside Git except for those explicit snapshots. Private
running notes, design proposals and literature-planning records are kept
separately. Existing project media remain with their reports; this consolidation
does not regenerate or relabel those demonstrations.
