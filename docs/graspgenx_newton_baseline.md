...
The two recorded comparison profiles are in
`config/dex3_newton_control_profiles.yaml`. Their values and source provenance
are separate from the later Isaac qualification profile. Structural agreement
with a source configuration does not establish retention; the simulation
outcome must come from the matching recorded trial.
